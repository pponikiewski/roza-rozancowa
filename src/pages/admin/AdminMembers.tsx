...
const MembersList = ({ list, onSelect, getMysteryName }: { list: AdminMember[], onSelect: (m: AdminMember) => void, getMysteryName: (id: number | null) => string }) => {
...
export default function AdminMembers() {
  ...
  const handleUpdateGroupClick = async () => {
    ...
    const success = await updateGroup(selectedMember.id, editGroupId)
    if (success) setSelectedMember(null)
  }

  const handlePasswordChangeClick = async () => {
    if (!selectedMember) return
    const success = await changePassword(selectedMember.id, newPassword)
  ...
  const groupedData = useMemo(() => {
    const map = new Map<number, AdminMember[]>(); const unassigned: AdminMember[] = []
          ...
          {selectedMember && (
            <div className="p-6 space-y-6 bg-card max-h-[80vh] overflow-y-auto [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-muted-foreground/20 [&::-webkit-scrollbar-thumb]:rounded-full">
              ...
              <div className="space-y-4">
                ...
                <div className="grid gap-2"><Label className="text-xs">Przypisanie do grupy</Label><div className="flex gap-2"><Select value={editGroupId} onValueChange={setEditGroupId}><SelectTrigger className="h-9 w-full text-sm"><SelectValue placeholder="-- Wybierz (lub usuń z grupy) --" /></SelectTrigger><SelectContent><SelectItem value="unassigned">-- Bez grupy (usuń) --</SelectItem>{groups.map(g => (<SelectItem key={g.id} value={g.id.toString()}>{g.name}</SelectItem>))}</SelectContent></Select><Button onClick={handleUpdateGroupClick} disabled={actionLoading} size="sm" variant="secondary" className="shrink-0"><RefreshCcw className="h-3.5 w-3.5 mr-2" /> Zmień</Button></div></div>
                <div className="grid gap-2">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button onClick={handlePasswordChangeClick} disabled={actionLoading || !newPassword} size="sm" variant="outline" className="shrink-0">Zapisz</Button>
                  </div>
                </div>
              </div>
              <Separator />
              <div className="flex justify-between items-center pt-2"><div className="text-xs text-muted-foreground">ID: <span className="font-mono">{selectedMember.id.substring(0, 8)}...</span></div><Button variant="destructive" size="sm" className="h-8 text-xs" onClick={handleDeleteUserClick} disabled={actionLoading}><Trash2 className="h-3.5 w-3.5 mr-2" /> Usuń konto</Button></div>
            </div>
          )}
        </DialogContent>
      </Dialog>
